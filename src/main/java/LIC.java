public class LIC {
    ...
    public static boolean LIC0(int numPoints, Point[] points, double LENGTH1) {
        for (int i = 0; i < numPoints - 1; i++) {
            if (Point.distance(points[i], points[i + 1]) > LENGTH1)
                return true;
        }
        return false;
    }
    ...
    public static boolean LIC1(int numPoints, Point[] points, double RADIUS1) {
        if (numPoints < 3 || RADIUS1 < 0)
            return false;
        for (int i = 0; i < numPoints - 2; i++) {
            if (Point.circleRadius(points[i], points[i + 1], points[i + 2]) > RADIUS1)
                return true;
        }
        return false;
    }
    ...
    public static boolean LIC6(int numPoints, Point[] points, int N_PTS, double DIST) {
        if (numPoints < 3 || N_PTS < 3 || DIST < 0 || numPoints < N_PTS)
            return false;

        for (int i = 0; i <= numPoints - N_PTS; i++) {
            Point p_start = points[i];
            Point p_end = points[i + N_PTS - 1];

            for (int j = i + 1; j <= i + N_PTS - 2; j++) {
                Point p_mid = points[j];
                if (Point.distancePointToLine(p_start, p_end, p_mid) > DIST) {
                    return true;
                }
            }
        }
    ...
    public static boolean LIC7(int numPoints, Point[] points, double LENGTH1, int K_PTS) {
        if (numPoints < 3 || K_PTS < 1 || K_PTS > numPoints - 2)
            return false;
        for (int i = 0; i < numPoints - K_PTS - 1; i++) {
            if (Point.distance(points[i], points[i + K_PTS + 1]) > LENGTH1)
                return true;
        }
        return false;
    }
    ...
    public static boolean LIC10(int numPoints, Point[] points, int E_PTS, int F_PTS, double AREA1) {
        if (numPoints < 5 || E_PTS < 1 ||F_PTS < 1 || E_PTS + F_PTS > numPoints - 3) return false;

        for (int i = 0; i < numPoints - F_PTS - E_PTS - 2; i ++) {
            Point p1 = points[i];
            Point p2 = points[i + E_PTS + 1];
            Point p3 = points[i + E_PTS + F_PTS + 2];
            
            if (Point.triangleArea(p1, p2, p3) > AREA1) {
                return true;
            }
        }
        return false;
